...
// src/utils.js
function clash(rawText) {
  try {
    const parsed = js_yaml_default.load(rawText);
    const proxies2 = parsed?.proxies;
    if (Array.isArray(proxies2)) {
      for (const p of proxies2) p.udp = true;
      return proxies2;
    }
  } catch {
  }
  ...
  for (const line of lines) {
    ...
    try {
      const obj = js_yaml_default.load("proxies:\n" + t);
      if (obj?.proxies?.[0]) {
    ...
    try {
      if (line.startsWith("vless://")) {
        ...
        const node = {
          ...
          servername: query.sni || void 0,
          flow: query.flow || void 0,
          "client-fingerprint": query.fp || void 0,
          "skip-cert-verify": query.allowInsecure === "1" ? true : void 0,
          network
        };
        ...
        const network = json2.net || "tcp";
        const node = {
          name: json2.ps || "",
          type: "vmess",
          server: json2.add,
          port: parseInt(json2.port),
          uuid: json2.id,
          alterId: parseInt(json2.aid) || 0,
          cipher: json2.scy || "auto",
          tls: json2.tls === "tls",
          "skip-cert-verify": false
        };
        if (json2.sni) node.servername = json2.sni;
        if (network === "ws") {
          node.network = "ws";
          node["ws-opts"] = {
            path: json2.path || "/",
            headers: {
              Host: json2.host || json2.add
            }
          };
        }
        proxies.push(node);
    ...
    try {
      if (o.type === "vless") {
        const node = {
          ...
          servername: o.tls?.server_name || void 0,
          "client-fingerprint": o.tls?.utls?.fingerprint || void 0
        };
        ...
        const node = {
          ...
          port: o.server_port,
          password: o.password,
          sni: o.tls?.server_name || void 0,
          alpn: o.tls?.alpn || void 0,
          tls: o.tls?.enabled === true,
...
rules:
  ...
  - GEOSITE,youtube,\u{1F30D}\u56FD\u5916\u5A92\u4F53\u{1F30D}

  - MATCH,\u{1F4CC}\u8282\u70B9\u9009\u62E9\u{1F4CC}


proxy-groups:
  - name: \u{1F4CC}\u8282\u70B9\u9009\u62E9\u{1F4CC}
    type: select
...
function setHeaders(upstreamHeaders, link) {
  ...
  if (cd) {
    ...
    if (m) {
      let n = m[1].trim();
      n = n.toLowerCase().startsWith("utf-8''") ? decodeURIComponent(n.slice(7)) : n.replace(/^["']|["']$/g, "");
      baseName = n.replace(/\.[^/.]+$/, "");
    }
  }
  if (!baseName) {
  ...
  const encode = /* @__PURE__ */ __name((s) => Array.from(new TextEncoder().encode(s)).map((b) => "%" + b.toString(16).toUpperCase().padStart(2, "0")).join(""), "encode");
